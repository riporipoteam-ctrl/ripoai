...
export function wantsPlaces(text: string): boolean {
  return /\b(near me|nearby|near by|near my location|near current location|using my location|close to me|by me|in my area|around here|local places?|nearest|closest|around me|where (is|are|can i)|find (me )?(a |the )?(nearest|closest|nearby)?|directions to|how do i get to|on a map|shopping mall|malls?|restaurants?|cafes?|coffee shops?|hotels?|bars?|gas station|petrol|pharmac(y|ies)|hospitals?|parks?|gyms?|stores? near|shops? near|atm|supermarket)\b/i.test(
    text,
  )
}
